...
- Rezdy moved behind Cloudflare Bot Management in September 2026 (before that, AWS WAF): both answer plain `curl` and Python TLS stacks with a 403/405 block page regardless of headers, which is why the scraper uses a browser-fingerprinted client for every site rather than a second HTTP stack for the two Rezdy sources.
